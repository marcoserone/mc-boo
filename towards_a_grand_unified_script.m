...
(*no check baby*)
mcIteratorNoCheck[\[CapitalDelta]\[Phi]_,initialOps_,finalOps_,\[CapitalDelta]Linitial_,\[Beta]_,nz_,prec_,seedO_,nits_,runid_,sigmaz_,sigmaMC_,maxReps_,sigmafac_]:=Block[{\[CapitalDelta]L=\[CapitalDelta]Linitial,sigmas=sigmaMC,results,repCount=0,checks,it,seed=seedO,nzeros=finalOps},
it=initialOps;
SetOptions[RandomReal,WorkingPrecision->100];
results=Reap[
While[it<=finalOps,
...
nzeros=checks[[3]];it=it+1;repCount=0;
(*Sigma updating*)
sigmas[[1;;it-initialOps]]=sigmas[[1;;it-initialOps]]sigmafac;
Print[sigmas],
If[repCount<maxReps,\[CapitalDelta]L[[1;;it,1]]=\[CapitalDelta]L[[1;;it,1]](1+Table[RandomReal[{-1/100,1/100}],{i,1,it}]);
Print["Rejected"];Print[checks[[2;;3]]];seed=seed+finalOps;repCount=repCount+1,Print["Andato_a"];Break[]]];
];
...
ParallelTable[
mcIteratorNoCheck[1,4,maxops,\[CapitalDelta]L,\[Beta]list,20,100,1+15i,nits,"sigmafac="<>ToString[i],1/10,a,1,100/(100+2i)]//Timing,{i,1,10}]
