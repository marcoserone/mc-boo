...
numerically troubling, since the behaviour of the logarithm around $0$ is far
from differentiable and the landscape was thus full of spurious minima out of
...
\begin{aligned}
  ...
  \log\Det^2\mathcal{M}(\{\Delta_i+\delta,l_i\})  +
  \log\Det^2\mathcal{M}(\{\Delta_i-\delta,l_i\})  \right)
\end{aligned}
\]
...
($\sim 4$) operators and add the next operators one by one. To avoid spoiling
...
reference here, other than Wikipedia?}. For a generic spectrum
...
fact that our spectrum is only approximate. Since the error estimate depends
